...
import { GameState, SERVE, TEAM_LEFT, TEAM_RIGHT, TEAM_NAMES } from './const';
...
export const startListen = (
    ...
    const internalObserver: ObserverInstance<GameState> = observerObj(
        bindArg(100, delayOperator)
    );
    internalObserver(
        bindArg((state: GameState) => {
            const {
                [TEAM_LEFT]: teamLeft,
