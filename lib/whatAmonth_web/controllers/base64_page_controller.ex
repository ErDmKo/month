defmodule WhatAmonthWeb.Base64PageController do
  ...
  def index(conn, %{"action" => "decode", "encodeQuery" => query})
      when byte_size(query) > 0 and
             byte_size(query) < 1000 do
    ctx = getCtx(query)

    ctx =
      with {:ok, result} <- Base.decode64(query) do
  ...
  def index(conn, %{"action" => "encode", "encodeQuery" => query})
      when byte_size(query) > 0 and
             byte_size(query) < 1000 do
    ctx = getCtx(query)
